...
class ShopCategories extends ADMIN_Controller
{
    ...
    public function index($page = 0)
    {
        ...
        if (isset($_GET['delete'])) {
            ...
            $this->session->set_flashdata('result_delete', '¡Se ha eliminado la categoría de la tienda!');
            redirect('admin/shopcategories');
        }
        if (isset($_POST['submit'])) {
            $this->Categories_model->setShopCategorie($_POST);
            $this->session->set_flashdata('result_add', '¡Se ha añadido la categoría de la tienda!!');
            redirect('admin/shopcategories');
        }
