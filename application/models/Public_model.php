...
class Public_model extends CI_Model
{
    ...
    public function getProducts($limit = null, $start = null, $big_get, $vendor_id = false)
    {
        ...
        #--- modificado edward se adiciona el campo products.procurement
        #--- modificado edward se adiciona las categorias 20200826-01
        $this->db->select('vendors.url as vendor_url, products.id,products.image, products.quantity, products_translations.title, products_translations.price, products_translations.old_price, products.url,products.procurement,shop_categories_translations.name');
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
    ...
    #--- Modificado Edward adiciono products.procurement
    public function getbestSellers($categorie = 0, $noId = 0)
    {
        $this->db->select('vendors.url as vendor_url, products.id, products.quantity, products.image, products.url, products_translations.price, products_translations.title, products_translations.old_price,products.procurement');
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
        $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        if ($noId > 0) {
    ...
    public function changePayuOrderStatus($order_id, $status, $payu_method_name)
    {
        $this->db->where('order_id', $order_id);
        if (!$this->db->update('orders', array(
                    'payu_status' => $status,
                    'payu_method_name' => $payu_method_name
                ))) {
            log_message('error', print_r($this->db->error(), true));
        }
    }
